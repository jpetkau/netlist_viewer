...
class Graph {
    ...
    insert_empty_cell(name: string | null, op: string, depth?: number): Cell {
        let c = {
            name,
            op,
        ...
        this.cells.push(c);
        this.layers[c.depth] = this.layers[c.depth] ?? [];
        this.layers[c.depth].push(c);
        return c;
    }

    make_visible(cell: Cell) { }
}

function cell_svg(cell: Cell): string {
    let img = "nand";
